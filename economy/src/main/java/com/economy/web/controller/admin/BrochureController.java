 ...
 *
 * @author: zhangteng
 * @time: 2014/4/30 15:08
 */
@Controller
@RequestMapping("admin/brochure")
public class BrochureController extends AbstractController {

    @ModelAttribute
    public void setMenu(Model model) {
        model.addAttribute("brochureMenuActived", true);
    }
    ...
    public String list(@RequestParam(value = "pageNo", required = false) Integer pageNo,
        ...
        try {
            ...
            int dataCount = articleService.queryCountByCondition(ArticleTypeEnum.Brochure.getType(), search);
            Pagination page = new Pagination(dataCount, this.pageSize, pageNo);
            List<Article> articleList = articleService.queryByCondition(page, search, ArticleTypeEnum.Brochure.getType());
    ...
    public String insert(ArticleDto articleDto) {
        try {
            articleDto.getArticle().setType(ArticleTypeEnum.Brochure.getType());
