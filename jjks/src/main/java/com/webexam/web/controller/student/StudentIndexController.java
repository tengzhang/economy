...
public class StudentIndexController extends AbstractController {
    ...
    public String chooseExam(Model model) {
        try {
            ...
            int[] isExamed = new int[examSize];
            for(int i = 0;i < examSize; ++i) {
                StudentSubject studentSubject = studentService.querySubjectBySidAndSubject(id, examList.get(i).getSubjectCode());
                if(studentSubject.getScore()==null ||
                        (studentSubject.getScore()!=null && studentSubject.getScore()==0)) {
                    isExamed[i] = 0;
                } else {
                    isExamed[i] = 1;
                }
